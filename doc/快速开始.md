...
1. 在“站点管理”创建站点，填写 Host、上游地址、运行模式和启用状态。
2. 在“规则管理”“策略”或模块页面启用需要验证的规则和策略。
3. 进入“发布记录”，点击“发布新版本”。创建站点或修改防护配置后必须发布，网关才会加载新配置。
4. 通过 Gateway 访问站点 Host，验证正常请求可以代理到上游。
...
- 日常后台操作、发布边界和排障见 [使用说明](使用说明.md)。
- 生产部署和安全加固见 [Debian 12 minimal 部署说明](Debian12部署说明.md)。
- 文档入口和历史规划说明见 [文档索引](文档索引.md)。
