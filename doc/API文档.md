...
应用字段：`name`、`mode`、`enabled`、`description`、`hosts`、`listeners`、`upstreams`、`proxy_config`。`mode` 支持 `monitor`、`protect`、`off`。
...
高级 nginx 配置用于少数需要直接调整 OpenResty/Nginx 指令的场景。普通反代行为优先使用防护应用的 `proxy_config`，不要为了设置 Header 或超时进入原始 nginx 编辑。
...
Dashboard 的“发布记录”页面会先调用 `GET /api/v1/releases/preview` 展示模块汇总、兼容诊断、`summary.advanced_nginx` 和风险提示，再调用 `POST /api/v1/releases` 发布。`summary.advanced_nginx` 包含是否存在高级 nginx 变更、校验状态、诊断、diff 和高风险警告。存在高级 nginx 变更时，发布请求必须带 `confirm_advanced_nginx=true`。
