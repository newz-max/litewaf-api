...
> 语言 / Language: [中文](CC防护使用指南.md) | [English](en/cc-protection.md)
...
列表支持 `application_id` 和 `enabled` 过滤。模拟预览接口只读取当前配置和请求样本，不会修改网关计数、发布记录或活动配置；返回命中的规则、计数维度、计数键说明、阈值窗口、动作和风险提示。
