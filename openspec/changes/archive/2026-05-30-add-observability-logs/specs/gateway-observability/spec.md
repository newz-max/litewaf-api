...
The gateway SHALL emit structured JSON WAF event logs for every rule, access-list, and rate-limit match that affects request disposition or is configured for observation.
...
The gateway SHALL avoid logging full request bodies, authorization headers, cookies, or configured sensitive headers in access and WAF event logs.
