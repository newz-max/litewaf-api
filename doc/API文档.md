...
创建、更新、删除或启停站点只会修改控制面数据；必须调用 `POST /api/v1/releases` 发布新版本后，网关才会按新站点路由和防护模式执行。发布前可调用 `GET /api/v1/releases/preview` 检查站点、规则、策略和模块摘要。
...
创建、更新、删除或启停策略后，需要发布新版本才会影响站点绑定规则、风险阈值和高级检测字段。
...
发布会生成网关配置并写入 `GATEWAY_CONFIG_PATH`。站点、规则、策略、IP 黑白名单、访问控制、限流、CC 防护、上传防护、Bot / 人机验证、动态防护和规则包导入后的可执行规则，都以发布版本作为网关生效边界；写入资源后未发布时，网关继续使用上一版活动配置。

Dashboard 的“发布记录”页面会先调用 `GET /api/v1/releases/preview` 展示模块汇总、兼容诊断和风险提示，再调用 `POST /api/v1/releases` 发布。`POST /api/v1/releases/{version}/rollback` 会把历史发布配置重新写入网关配置路径，并创建新的回滚发布记录；它不会撤销当前数据库中的站点、规则或策略编辑。
...
| POST | `/api/v1/dynamic-bans/unban` | 写 | 手动释放站点/IP 动态封禁 |
...
活动动态封禁来自网关上报的 `event_type=dynamic-ban` WAF 事件，API 会维护站点/IP 维度的活动封禁投影。列表接口支持 `site_id`、`client_ip`、`status`、`limit` 和 `offset` 过滤；过期或已清理的封禁不会作为默认活动封禁返回。

```bash
curl -H "Authorization: Bearer <token>" "http://localhost:18080/api/v1/dynamic-bans?site_id=1&client_ip=203.0.113.41"
```

返回字段包括 `site_id`、`client_ip`、`ban_reason`、`source`、`source_event_id`、`ban_duration_sec`、`ban_remaining_sec`、`status`、`revision`、`created_at`、`expires_at`、`cleared_at` 和 `updated_at`。无匹配数据时返回空 `items`，不返回演示行。

管理员可按站点和有效客户端 IP 手动释放封禁：
...
释放结果包含 `site_id`、`client_ip`、`status`、`revision`、`cleared_at` 和 `message`。`status=cleared` 表示存在活动封禁并已标记清理；`status=no-op` 表示目标不存在、已过期或已清理，接口仍会生成清理 revision 供网关消费，便于重复点击或重试时保持幂等。readonly 和 auditor 角色只能查看，不能调用释放接口。
...
该接口只返回有界站点/IP、状态、revision 和消息，不包含请求体、Cookie、Authorization、网关令牌或动态 token。网关在 worker 定时同步后删除本地 `site_id/client_ip` 对应动态封禁 key，请求热路径不调用控制面 API；实际解除阻断存在一个同步间隔内的传播延迟。
