...
应用字段：`name`、`mode`、`enabled`、`description`、`hosts`、`listeners`、`upstreams`、`routes`、`proxy_config`。`mode` 支持 `monitor`、`protect`、`off`。
...
- `routes[]`：可选 URL 路由，字段为 `name`、`path`、`path_match`、`upstream_name`、`priority`、`enabled`、`proxy_config`。
- `proxy_config`：可选对象，缺省时保持默认反代行为。支持 `headers[]`、`connect_timeout`、`read_timeout`、`send_timeout`、`websocket_enabled`、`preserve_host`、`proxy_buffering`、`request_buffering`。

`proxy_config.headers[]` 字段为 `name`、`value`。Header 名称只允许字母、数字、下划线和短横线；Header 值不允许换行。超时值使用 Nginx 时间格式，例如 `500ms`、`30s`、`5m`。`proxy_buffering` 和 `request_buffering` 可为 `on`、`off` 或空字符串。`preserve_host=false` 时发布配置会把上游 `Host` 设置为 `$proxy_host`，缺省和 `true` 时保持 `$host`。

`routes[].path_match` 支持 `exact`、`prefix`、`glob`，路径必须以 `/` 开头，glob 使用 LiteWaf 统一路径匹配限制。`routes[].upstream_name` 必须引用同一应用内存在的 upstream 名称；启用路由不能指向已停用 upstream。多个路由按 `priority` 从小到大确定匹配顺序，`priority` 必须大于 0 且同一应用内不能重复。没有配置启用路由或请求未命中任何路由时，网关保持兼容行为：使用第一个启用 upstream 作为默认兜底。

路由级 `proxy_config` 会随应用保存和发布进入网关配置数据，用于表达路由专属代理行为；当前生成的 nginx listener 仍保留单个 `location /`，因此超时、buffering 等 nginx 静态指令按应用级 `proxy_config` 生效，URL 路由本身负责在 Lua 热路径中选择目标 upstream。需要任意 nginx 指令、额外 location 或复杂 rewrite 时，应继续使用高级 nginx 草稿能力。

HTTPS 监听启用时必须绑定可用证书。创建、更新、删除或启停防护应用只会修改控制面数据；必须调用 `POST /api/v1/releases` 发布新版本后，网关才会按新的应用入口、端口、协议、URL 路由和上游执行。

## 高级 nginx 配置草稿

高级 nginx 配置用于少数需要直接调整 OpenResty/Nginx 指令的场景。普通反代行为优先使用防护应用的结构化域名、监听、upstream、`routes` 和 `proxy_config`，不要为了设置常规 URL 到上游映射、Header 或超时进入原始 nginx 编辑。

| 方法 | 路径 | 权限 | 说明 |
| --- | --- | --- | --- |
| GET | `/api/v1/nginx-config` | 读 | 查询已保存的高级 nginx 草稿 |
| GET | `/api/v1/nginx-config/effective` | 读 | 查询当前生效的 runtime nginx 配置 |
| PUT | `/api/v1/nginx-config` | 写 | 保存高级 nginx 草稿 |
| POST | `/api/v1/nginx-config/validate` | 写 | 构造有效配置并执行 nginx 校验 |

草稿字段：

- `mode`：`snippets` 或 `full`。
- `snippets[]`：受控片段，字段为 `include_point` 和 `content`，`include_point` 支持 `http`、`server`、`location`。
- `full_config`：完整 `nginx.conf` 覆盖内容，仅 `mode=full` 时生效。
- `validation`：最近一次有效配置校验结果，包含 `status`、`message`、`diagnostics`、`validated_at`。
- `updated_by`、`updated_at`：最近保存人和保存时间。

生效配置字段：

- `source`：配置来源，可能为 `runtime_full`、`runtime_snippets`、`generated_default` 或 `missing`。
- `mode`：`snippets` 或 `full`。
- `snippets[]`：当前 runtime 片段内容，仅 snippet 来源时有内容。
- `full_config`：当前 runtime `nginx.conf` 或基于 listener artifacts 生成的默认完整配置。
- `runtime_dir`、`config_path`：后端读取 runtime artifacts 时使用的路径元数据。

草稿接口只返回已保存的待发布内容，不再用 runtime artifacts 填充编辑态。Dashboard 如需展示当前网关正在使用的 nginx 配置，应读取 `/api/v1/nginx-config/effective`；如需基于当前生效配置继续编辑，应由用户显式复制到草稿后再保存。

草稿允许保存失败或未校验内容；发布时如果存在 snippet 或 full override 变更，必须满足以下条件：有效配置校验通过、发布预览展示 diff 和高风险警告、`POST /api/v1/releases` 请求体包含 `confirm_advanced_nginx=true`。如果 `LITEWAF_NGINX_VALIDATE_COMMAND` 未配置或校验失败，发布会被阻断且不会写入活动 runtime artifacts。

完整 `nginx.conf` 覆盖是最高风险模式。后端会检查必须保留 LiteWaf 健康检查、metrics、Lua WAF hook、listener include、运行时 upstream 变量和 `proxy_pass $litewaf_upstream` 等不变量；缺失时即使命令校验可用也不能发布。
...
Dashboard 的“发布记录”页面会先调用 `GET /api/v1/releases/preview` 展示模块汇总、兼容诊断、`summary.routes`、`summary.enabled_routes`、`summary.application_validation`、`summary.advanced_nginx` 和风险提示，再调用 `POST /api/v1/releases` 发布。`summary.application_validation` 会报告路由目标缺失、启用路由指向停用 upstream、路径匹配非法或优先级重复等阻断问题。`summary.advanced_nginx` 包含是否存在高级 nginx 变更、校验状态、诊断、diff 和高风险警告。存在高级 nginx 变更时，发布请求必须带 `confirm_advanced_nginx=true`。

`POST /api/v1/releases/{version}/rollback` 会把历史发布配置和该版本保存的 runtime artifact snapshot 重新写入网关配置路径，并创建新的回滚发布记录；它不会撤销当前数据库中的防护应用、规则或策略编辑。

发布预览的 `summary.gateway.client_max_body_size` 返回当前网关请求体硬上限。发布和回滚会在 `GATEWAY_CONFIG_PATH` 同级运行时目录写入 `listeners/body-size.conf`，内容为 `client_max_body_size <value>;`。该限制属于 L1 网关请求体硬上限，在 OpenResty 请求体读取阶段生效，早于上传防护和其他 LiteWaf Lua 规则。

上传大小相关限制统一按分层模型解释：

| 层级 | 名称 | 执行阶段 | 超限表现 | 是否产生 WAF 事件 |
| --- | --- | --- | --- | --- |
| L0 | 前置代理请求体上限 | LiteWaf 之前 | 前置代理返回 413 或自定义错误 | 否 |
| L1 | 网关请求体硬上限 | OpenResty 读取请求体阶段 | OpenResty 原生 413 | 否 |
| L2 | 请求体检测读取上限 | LiteWaf Lua 检测阶段 | 按策略观察、跳过或阻断 | 可产生 |
| L3 | 上传防护大小规则 | 上传防护模块规则阶段 | LiteWaf 定义的阻断/观察响应 | 可产生 |

`GET /api/v1/version` 和 `GET /api/v1/releases/preview` 都会返回只读 `upload_limits` 摘要，用于 Dashboard 展示当前 LiteWaf 可控层级的值、来源、执行阶段、事件可见性和风险提示。L0 前置代理限制只在文档和排障建议中解释，LiteWaf 不声称能读取或管理外部代理的实际值。
...
发布配置顶层输出 `applications`，不再输出旧 `sites` 字段。每个应用包含 `hosts`、`listeners`、`upstreams`、`routes` 和应用级 `proxy_config`；网关先按 Host、监听端口和协议选中应用，再按 `routes[].priority` 的顺序匹配 URL 路径并选择 `upstream_name` 指向的启用 upstream，未命中时回退到第一个启用 upstream。发布配置会保留旧 `rate_limits` 限流兼容字段，并输出独立 `ip_access_index` 作为 IP 黑白名单运行时索引；旧 `access_lists` 字段、接口和网关 fallback 已移除。`protection_rules` 表继续输出 CC 防护、访问控制、上传防护、Bot / 人机验证和动态防护子集。托管攻击防护规则继续位于应用 `rules` 数组中，同时带有网关可识别的 `module=attack-protection`、`category=managed`、`attack_type`、`group` 和 `priority` 元数据：
...
发布预览的 `summary.gateway` 包含网关运行时硬限制，例如请求体入口上限。`summary.upload_limits` 包含 L1/L2/L3 上传限制摘要和结构化 `warnings`：当网关硬上限小于上传防护大小规则、请求体检测读取上限小于上传防护大小规则，或网关硬上限低于常见上传场景阈值时返回非阻断风险提示。提示只用于发布前解释，不改变网关执行结果，也不会把 OpenResty 原生 413 伪装成上传防护命中。`summary.ip_access_list` 包含 IP 黑白名单总数、启用数量、白名单/黑名单数量、Exact IP 数量、CIDR 数量、全局和站点作用域数量。`summary.cc_protection` 包含 CC 规则总数、启用数量和高风险配置提示。`summary.attack_protection` 包含攻击防护组数量、启用数量、观察数量、阻断数量和受影响攻击类型。`summary.access_control` 包含访问控制规则总数、启用数量、允许/观察/阻断数量和宽泛允许类风险提示。`summary.upload_protection` 包含上传防护规则总数、启用数量、扩展名规则数、大小规则数、观察/阻断数量和高风险上传限制提示。`summary.bot_protection` 包含 Bot 规则总数、启用数量、JS challenge 数量、阻断数量、观察数量和宽泛 challenge 提示。`summary.dynamic_protection` 包含动态防护规则总数、启用数量、动态令牌数量、页面动态化数量、等候室数量、阻断数量、观察数量、等候室动作数量和宽泛路径提示。
