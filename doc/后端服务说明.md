...
部署时不要把商业或生产 GeoIP 数据提交到仓库；应通过宿主机文件或只读 volume 挂载到容器内，再设置 `LITEWAF_GEOIP_DB_PATH=/path/in/container/geoip.csv`。未配置、文件不可读、IP 无匹配或私有/保留地址时，系统不会生成假地区，统计报表会返回空地理数据和诊断信息。
