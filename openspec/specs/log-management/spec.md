...
The API SHALL expose protected ingestion endpoints for gateway access logs and WAF event logs, including advanced protection event fields.
...
#### Scenario: Advanced WAF event ingestion succeeds
- **WHEN** an authorized gateway submits a WAF event containing score, normalized target, body metadata, upload metadata, or dynamic-ban fields
- **THEN** the API stores those fields in the configured log storage backend when present
...
The API SHALL expose WAF event or attack log query endpoints under `/api/v1` with filters for time range, site, client IP, rule, action, disposition, event type, advanced target, and minimum score.
...
#### Scenario: Query attack logs by advanced target
- **WHEN** an allowed user queries attack logs with a body, upload, score, or dynamic-ban event type filter
- **THEN** the API returns only WAF event records matching that advanced protection dimension

### Requirement: Log summaries are available
The API SHALL expose summary endpoints that aggregate stored logs for dashboard views, including advanced protection counters.
...
#### Scenario: Dashboard requests advanced protection summary
- **WHEN** an allowed user requests observability summary data
- **THEN** the API includes counts for score-threshold blocks, body detections, upload detections, and dynamic-ban events when present
