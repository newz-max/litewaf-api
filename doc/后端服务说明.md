...
| GET | /api/v1/access-lists | 黑白名单兼容列表 |
| POST | /api/v1/access-lists | 创建黑白名单兼容记录 |
...
`/api/v1/rate-limits` 和 `/api/v1/access-lists` 是旧入口兼容 API，继续保留给旧客户端、历史数据排障、发布回滚和网关 fallback。后台常规新增 URL 频率限制、登录防爆破和 API 限流应优先使用 CC 防护；常规新增 IP/CIDR、路径、Header、Host 的允许、观察或阻断规则应优先使用访问控制。
